<template>
    <form @submit.prevent="proceed" @keydown="todoForm.errors.clear($event.target.name)">
        <div class="row">
            <div class="col-12 col-sm-4">
                <div class="form-group">
                    <label for="">{{trans('todo.Bedrijfsnaam')}}</label>
                    <input class="form-control" type="text" value="" v-model="todoForm.title" name="title" :placeholder="trans('todo.Bedrijfsnaam')">
                    <show-error :form-name="todoForm" prop-name="title"></show-error>
                </div>
            </div>
            <div class="col-12 col-sm-4">
                <div class="form-group">
                    <label for="">{{trans('todo.Geboortedatum')}}</label>
                    <datepicker v-model="todoForm.date" :bootstrapStyling="true" @selected="todoForm.errors.clear('date')" :placeholder="trans('todo.Geboortedatum')"></datepicker>
                    <show-error :form-name="todoForm" prop-name="date"></show-error>
                </div>
            </div>
            <div class="col-12 col-sm-4">
                <div class="form-group">
                    <label for="">{{trans('todo.Opmerking')}}</label>
                    <autosize-textarea v-model="todoForm.description" rows="1" name="description" :placeholder="trans('todo.Opmerking')"></autosize-textarea>
        ...
        <router-link to="/proefritten" class="btn btn-danger waves-effect waves-light pull-right m-r-10" v-show="id">{{trans('general.cancel')}}</router-link>
