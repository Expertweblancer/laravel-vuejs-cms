            ...
            <div class="row">
                ...
                <div class="col-3">
                    <h3 class="text-themecolor">
                        <sort-by class="pull-right" :order-by-options="orderByOptions" :sort-by="filterTodoForm.sort_by" :order="filterTodoForm.order" @updateSortBy="value => {filterTodoForm.sort_by = value}"  @updateOrder="value => {filterTodoForm.order = value}"></sort-by><br>
                        <button class="btn btn-info btn-sm pull-right m-r-10 new_todo" v-if="todos.total && !showCreatePanel" @click="showCreatePanel = !showCreatePanel"><i class="fas fa-plus"></i> <span class="d-none d-sm-inline">{{trans('todo.add_new_todo')}}</span></button>
                    </h3>
                </div>    
            </div>
        </div>
                        ...
                        <div class="card-body">
                            <div class="table-responsive" v-if="todos.total">
                                <table class="table">
                                    <thead>
                                        <tr>
                                            <th>{{trans('todo.Bedrijfsnaam')}}</th>
                                            <th>{{trans('todo.Geboortedatum')}}</th>
                                            <th>{{trans('todo.status')}}</th>
                                            <th>{{trans('todo.completed_at')}}</th>
                                            <th>{{trans('todo.Opmerking')}}</th>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editTodo(todo){
                this.$router.push('/proefritten/'+todo.id+'/edit');
            },
            confirmDelete(todo){
